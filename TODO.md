...
- [ ] **Download / Materiali Distrettuali**: il bottone in header "Materiali Distrettuali" punta a `#` (placeholder). Sul sito attuale è una pagina che linka a una cartella Google Drive condivisa — direzione scelta: pagina dedicata con listing brandizzato, vedi voce sotto in "Nuove funzionalità richieste".
...
- [ ] **Pagina Download / Materiali Distrettuali**: documenti vivono in una cartella Google Drive condivisa del distretto. Vogliono i file mostrati "wrappati" in stile sito (niente iframe grezzo, niente semplice redirect/link esterno) e il più automatico possibile (nessun rebuild manuale quando aggiungono un file). Direzione scelta: **fetch client-side** (isola `client:load`, non build-time) della Drive API v3 (`files.list` sulla cartella) con API key ristretta (solo Drive read-only, referrer-locked al dominio del sito) → resa come lista/grid brandizzata (icona per tipo file, nome, dimensione, link download che apre il file su Drive). Aggiornamento istantaneo per ogni visitatore appena la cartella cambia, zero infra da mantenere (niente webhook/cron/build hook). Unica pagina del sito non zero-JS/pre-renderizzata, stessa eccezione già presa per la mappa club. Scartata l'alternativa webhook Drive→Netlify build hook (serve funzione serverless + rinnovo periodico subscription Drive ogni ~7gg, troppa manutenzione per sito gestito da volontari). **Serve dal distretto**: ID/link della cartella Drive condivisa.
