...
Il progetto Astro+Tina **reale e funzionante vive alla root del repo** (non in una sottocartella) — non è più solo un mockup. È già stato implementato seguendo il pattern a blocchi descritto sotto: `tina/config.ts` (schema), `src/pages/[...slug].astro` + `src/pages/en/[...slug].astro` (route), `src/components/BlockRenderer.astro` + `src/components/blocks/*.astro` (19 blocchi), `src/content/{pages,zones,clubs,settings}/` (contenuti git-backed, `pages` a file unico per lingua dal 2026-08-16 — vedi `references/tina.md`). Leggere questi file **prima** di aggiungere qualcosa di nuovo: sono l'esempio concreto da estendere, più affidabile di qualunque descrizione astratta in questa skill. Il `README.md` alla root riassume struttura e comandi.
