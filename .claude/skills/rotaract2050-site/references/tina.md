...
- **IT/EN: un file solo con campi `xEn`, non due file separati** — pattern usato da `clubs`/`zones` (`story`/`storyEn`, `description`/`descriptionEn`) ed esteso a `news`/`events` il 2026-08-04 (`title`/`titleEn`, `excerpt`/`excerptEn`, `imageLabel`/`imageLabelEn`, `body`/`bodyEn` su news). Motivo: file separati per lingua (`it/slug.md` + `en/slug.md`) sono scomodi per editor non tecnici e il contenuto derivava facilmente (visto un caso reale: un evento con un tag club presente solo nella versione IT). Lettura per lingua tramite helper `localizeNews`/`localizeEvent` (`src/lib/news.ts`, `src/lib/events.ts`): ritornano il campo EN se compilato, altrimenti fallback su IT — stesso schema di `ClubDetail.astro` (`story`/`storyEn`). **`pages` resta l'eccezione**, ancora due file per lingua (`src/content/pages/it/`, `src/content/pages/en/`, vedi `references/astro.md`): stesso merge sarebbe corretto ma è un lavoro molto più grosso (11 template a blocchi, molti campi annidati, `BlockRenderer.astro`, entrambe le route) — deferito, non ancora pianificato.
