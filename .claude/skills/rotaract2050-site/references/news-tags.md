...
2. **`scope`** (`type: 'string', list: true, options: ['Distretto', 'MDIO']`) — checkbox multiplo con due valori fissi:
   - `Distretto` — news di interesse per tutto il Distretto 2050.
   - `MDIO` — tag esatto richiesto dal distretto (i soci lo riconoscono così, non espanderlo/rinominarlo in UI o contenuti).
   Un articolo può avere entrambi, uno solo, o nessuno dei due (se riguarda solo club specifici via `clubs`).
